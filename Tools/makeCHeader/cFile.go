...
func MakeCFunc(name, returnType string, params []Param, isClass bool) {

	if name == "DSendMessage" || name == "DCreateURLShortCut" {
		cFile.WLn()
		cFile.WLn()
		if name == "DSendMessage" {
			cFile.W("#ifndef _WIN32\n")
		} else if name == "DCreateURLShortCut" {
			cFile.W("#ifdef _WIN32\n")
		}
	...
	cFile.W(fmt.Sprintf("DEFINE_FUNC_PTR(%s)\n", name))
	if returnType != "" {
		cFile.W(cTypeConvert(returnType))
	} else {
		cFile.W("void")
	}
	cFile.W(" ")
	...
	cFile.W(") {\n")
	cFile.W(fmt.Sprintf("    GET_FUNC_ADDR(%s)\n", name))
	cFile.W("    ")
	if returnType != "" {
		cFile.W(fmt.Sprintf("return (%s)", cTypeConvert(returnType)))
	}
	cFile.W(fmt.Sprintf("MySyscall(p%s, %d%s);", name, len(params), getParamNames()))
	cFile.WLn()
	cFile.W("}\n")
	cFile.WLn()
